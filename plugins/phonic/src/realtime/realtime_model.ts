 ...
 * {@link RealtimeModel.updateConfig} / {@link RealtimeSession.updateConfig}. Connection-level fields
...
export class RealtimeModel extends llm.RealtimeModel {
  ...
  /** @internal the live session, used to forward mid-session {@link updateConfig} calls */
   ...
   * {@link RealtimeSession.updateConfig}, which applies the change immediately via a Phonic `reset`.
...
export class RealtimeSession extends llm.RealtimeSession {
   ...
   * history and send a `reset`, so a mid-session change (an Agent handoff via {@link _updateSession}
   * or a config change via {@link updateConfig}) takes effect. No-op if the socket isn't open yet.
   */
   ...
   * switch the language (or any other field) for the next question.
   */
  async updateConfig(config: PhonicSessionConfigUpdate): Promise<void> {
    if (this.closed) {
